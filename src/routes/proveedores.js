...
router.get('/delete/:cod/:borrar',async(req,res)=>{
    ...
    if(borrar=="1"){
        await pool.query('delete from db_partent.proveedor WHERE cod =?',[parseInt(cod)]);        

        res.redirect('/proveedores');
    }
    await pool.query('update db_partent.proveedor SET est=not est WHERE cod =?',[cod]);
...
router.get('/edit/:cod',async(req,res)=>{ //se muestra en el link 
    ...
    var proveedores = await pool.query('SELECT * FROM proveedor WHERE cod=?',[cod]); //lo va a devolver en un arreglo de uno porque pos solo hay uno con un cod 
